...
const FIELDS = [
	...
	["showAttention", "Needs Attention summary"],
	["showUsage", "Usage panel"],
	["showRecentChanges", "What changed"],
	["showWaits", "Waits"],
		...
		api.settings.describe({}).then((response) => {
			...
			if (section !== void 0) {
				...
				setCurrent({
					...
					showGoals: value?.showGoals ?? true,
					showWorkflows: value?.showWorkflows ?? true,
					showSchedules: value?.showSchedules ?? true,
					showAttention: value?.showAttention ?? true,
					showUsage: value?.showUsage ?? true,
					showRecentChanges: value?.showRecentChanges ?? true,
...
//#endregion
//#region src/client/attention.ts
/**
...
function deriveAttention(input) {
	...
	if (input.goal?.phase === "blocked") {
		...
		items.push({
			...
			severity: "warning",
			reason: "blocked",
			explanation: message === void 0 ? "Goal is blocked." : message,
			target: { kind: "goal" }
		});
	}
	for (const [sessionId, jobs] of Object.entries(jobsBySession)) for (const job of jobs ?? []) {
	...
	if (!input.teamUnavailable) for (const task of tasks) {
		if (task.status !== "pending" && task.status !== "in_progress") continue;
		const openBlockers = task.blockedBy.filter((id) => !tasks.some((other) => other.id === id && (other.status === "completed" || other.status === "deleted")));
	...
	for (const item of schedulesOverdue(schedules, input.nowMs)) items.push({
...
function schedulesOverdue(schedules, nowMs) {
	return schedules.filter((item) => {
		if (item.kind === "after" || item.dispatched || item.scheduledAt === void 0) return false;
		return Date.parse(item.scheduledAt) <= nowMs;
	});
}
//#endregion
//#region src/client/delta.ts
const FAILURE_JOB_STATUSES = /* @__PURE__ */ new Set(["failed", "error"]);
/**
...
*/
function deriveOperationsDelta(domain, anchor, nowMs) {
	const items = [];
	if (anchor.kind === "first-visit") return {
		since: anchor,
		generatedAt: nowMs,
		items
	};
	const since = anchor.at;
	const after = (at) => at !== void 0 && at > since && at <= nowMs;
	for (const { sessionId, job } of domain.jobs ?? []) {
		const id = String(job.id);
		if (after(job.startedAt)) items.push({
		...
		if (after(job.finishedAt)) items.push({
	...
	for (const wait of domain.waits ?? []) if (after(wait.createdAt)) items.push({
	...
	for (const group of domain.groups ?? []) if (after(group.createdAt)) {
	...
	if (domain.goal?.updatedAt !== void 0 && after(domain.goal.updatedAt)) items.push({
		id: "goal-updated",
		entityKind: "goal",
		entityId: domain.goal.objective ?? "goal",
		change: `Goal ${domain.goal.phase ?? "updated"}${domain.goal.blockedReason === void 0 ? "" : ` — ${domain.goal.blockedReason.message}`}`,
		severity: domain.goal.phase === "blocked" ? "attention" : "info",
		authoritativeAt: domain.goal.updatedAt
	});
	for (const item of domain.schedules ?? []) {
		if (item.dispatched || item.scheduledAt === void 0 || item.kind === "after") continue;
		const target = Date.parse(item.scheduledAt);
		if (!Number.isFinite(target)) continue;
		if (target > since && target <= nowMs) items.push({
			id: `schedule-overdue:${item.id}`,
			entityKind: "schedule",
			entityId: item.id,
	...
	for (const message of domain.messages ?? []) if (after(message.queuedAt)) items.push({
	...
	items.sort((left, right) => (right.authoritativeAt ?? 0) - (left.authoritativeAt ?? 0) || (left.id < right.id ? -1 : left.id > right.id ? 1 : 0));
	return {
		since: anchor,
		generatedAt: nowMs,
		items
...
*/
function mapSearchResults(items, agents) {
	...
	return items.map((item) => {
		...
		return {
			...
			...agent === void 0 ? {} : { label: agent.label },
			...agent === void 0 ? {} : { target: {
				kind: "agent",
				sessionId: item.sessionId
			} }
...
function usageRows(usage) {
	if (usage === null || usage === void 0) return [];
	const rows = [];
	if ((usage.uncachedInputTokens ?? 0) > 0) rows.push({
		key: "uncachedInputTokens",
		label: "input (uncached)",
		tokens: usage.uncachedInputTokens
	});
	if ((usage.cacheReadTokens ?? 0) > 0) rows.push({
		key: "cacheReadTokens",
		label: "cache read",
		tokens: usage.cacheReadTokens
	});
	if ((usage.cacheWriteTokens ?? 0) > 0) rows.push({
		key: "cacheWriteTokens",
		label: "cache write",
		tokens: usage.cacheWriteTokens
	});
	if ((usage.outputTokens ?? 0) > 0) rows.push({
		key: "outputTokens",
		label: "output",
...
function formatTokens(tokens) {
	...
	if (tokens < 1e4) return String(tokens);
...
function isWorkspaceEntityKind(value) {
	return value === "general" || value === "agent" || value === "job" || value === "job-group" || value === "wait" || value === "task" || value === "goal" || value === "workflow" || value === "schedule" || value === "session-view";
}
...
function WorkspaceView(props) {
	const { sessionId, useSessions, useProjection, api, rpc, config, refreshSubagents, openChild, sessionViews, search } = props;
	const SessionSlotHost = props.SessionSlotHost;
	...
	const goal = useProjection("goal") ?? null;
	const workflows = useProjection("goodjob/workflows")?.runs ?? [];
	const schedules = useProjection("goodjob/schedules")?.schedules ?? [];
	const usage = useProjection("tokenUsage") ?? void 0;
...
function SearchPanel(props) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			...
			phase.state === "loading" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: "gj-quiet",
				role: "status",
				children: "Searching…"
			}) : null,
			phase.state === "done" || phase.state === "partial" ? phase.results.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: "gj-quiet",
				children: "No matching sessions."
			}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
				className: "gj-list gj-searchResults",
				children: phase.results.map((hit) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", {
					className: "gj-listRow gj-searchRow",
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						type: "button",
						className: "gj-linkButton gj-rowMain",
						disabled: hit.target === void 0,
						title: `${hit.label ?? hit.sessionId} · ${hit.snippet}`,
						onClick: () => {
							if (hit.target !== void 0) props.onOpen({
								kind: "agent",
								sessionId: hit.sessionId
							});
						},
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: hit.label ?? hit.sessionId }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: "gj-meta",
			...
			phase.state === "partial" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: "gj-quiet",
				children: "More matches exist; refine the query."
			}) : null,
			phase.state === "unavailable" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: "gj-quiet",
				children: "Session search is unavailable in this deployment."
			}) : null,
			phase.state === "error" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
				className: "gj-quiet",
				children: ["Search failed: ", phase.message]
			}) : null
		]
	});
}
function Explorer(props) {
	...
	const goals = props.config.showGoals && props.domain.goal?.objective !== void 0 ? [{
		id: props.domain.goal.id ?? "goal",
		objective: props.domain.goal.objective
	}].filter((goal) => matches(`${goal.objective} ${goal.id}`)) : [];
	const workflows = (props.domain.workflows ?? []).filter((run) => props.config.showWorkflows).filter((run) => matches(`${run.name} ${run.id}`));
	const schedules = (props.domain.schedules ?? []).filter((item) => props.config.showSchedules).filter((item) => matches(`${item.prompt} ${item.id}`));
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("aside", {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)(SearchPanel, {
				agents: leadAndAgents(props.domain),
				search: props.search,
				onOpen: props.onOpen
			}),
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					props.config.showGoals && props.domain.goal?.objective !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ExplorerSection, {
						label: "Goals",
						kind: "goal",
						collapsed: props.collapsed,
						onToggle: props.onToggle,
						children: goals.map((goal) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ExplorerRow, {
					...
					props.config.showWorkflows ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ExplorerSection, {
						label: "Workflows",
						kind: "workflow",
						collapsed: props.collapsed,
						onToggle: props.onToggle,
						children: workflows.map((run) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ExplorerRow, {
							label: run.name,
					...
					props.config.showSchedules ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ExplorerSection, {
						label: "Schedules",
						kind: "schedule",
						collapsed: props.collapsed,
						onToggle: props.onToggle,
						children: schedules.map((item) => {
							const overdue = item.kind !== "after" && !item.dispatched && item.scheduledAt !== void 0 && Date.parse(item.scheduledAt) <= Date.now();
							return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ExplorerRow, {
								label: item.prompt,
...
function EntityEditor(props) {
	...
	switch (entity.kind) {
		...
		case "goal": return domain.goal?.objective === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(UnavailableEditor, { entity }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(GoalEditor, {
			...props,
			goal: domain.goal
		});
		case "workflow": {
			const run = (domain.workflows ?? []).find((candidate) => candidate.id === entity.workflowId);
			return run === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(UnavailableEditor, { entity }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(WorkflowEditor, {
				...props,
				run
			});
		}
		case "schedule": {
			...
			return item === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(UnavailableEditor, { entity }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ScheduleEditor, {
				...props,
				item
			});
		}
...
function WhatChanged(props) {
	...
	const visitStartedAt = (0, react.useRef)(Date.now());
	const [anchorAt, setAnchorAt] = (0, react.useState)(() => {
		const raw = props.storage?.getItem(storageKey);
		const parsed = raw === void 0 || raw === null || raw === "" ? NaN : Number(raw);
		return Number.isFinite(parsed) && parsed > 0 ? parsed : void 0;
	});
	const anchor = anchorAt === void 0 ? { kind: "first-visit" } : {
		kind: "last-visit",
		at: anchorAt
	};
	const delta = (0, react.useMemo)(() => deriveOperationsDelta(props.domain, anchor, Date.now()), [props.domain, anchorAt]);
	(0, react.useEffect)(() => () => {
		const raw = props.storage?.getItem(storageKey);
	...
	const entityFor = (item) => {
		switch (item.entityKind) {
			case "job": return item.sessionId === void 0 ? void 0 : {
				kind: "job",
				sessionId: item.sessionId,
				jobId: item.entityId
			};
			case "wait": return {
				kind: "wait",
				waitId: item.entityId
			};
			case "job-group": return {
				kind: "job-group",
			...
			case "schedule": return {
				kind: "schedule",
				scheduleId: item.entityId
			};
			case "message": return item.sessionId === void 0 ? void 0 : {
				kind: "agent",
				sessionId: item.sessionId
			};
		}
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		...
		children: [
			...
			anchor.kind === "first-visit" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: "gj-quiet",
				children: "First visit in this browser — changes are tracked from now on."
			}) : delta.items.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: "gj-quiet",
				children: "Nothing authoritative changed since the last visit."
			}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
				className: "gj-list",
				"aria-label": "Operations delta",
				children: delta.items.map((item) => {
					...
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "gj-badge",
								"data-severity": item.severity === "failure" ? "error" : item.severity === "attention" ? "warning" : "info",
								children: item.entityKind
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "gj-linkButton gj-rowMain",
								disabled: entity === void 0,
								onClick: () => {
									if (entity !== void 0) props.onOpen(entity);
								},
								children: item.change
							}),
							item.authoritativeAt === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("time", {
								className: "gj-meta",
								children: new Date(item.authoritativeAt).toLocaleTimeString()
							})
						]
					}, item.id);
				})
			}),
			anchor.kind === "last-visit" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
				className: "gj-quiet",
				children: [`Since last visit (${new Date(anchor.at).toLocaleString()}) · derived only from authoritative timestamps · `, /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
...
function GeneralEditor(props) {
	...
	const attentionEntities = /* @__PURE__ */ new Map();
	for (const item of attention) {
		const target = item.target;
		if (target.kind === "job") attentionEntities.set(item.id, {
			kind: "job",
			sessionId: target.sessionId,
			jobId: target.jobId
		});
		else if (target.kind === "goal") attentionEntities.set(item.id, { kind: "goal" });
		else if (target.kind === "wait") attentionEntities.set(item.id, {
			kind: "wait",
			waitId: target.waitId
		});
		else if (target.kind === "task") attentionEntities.set(item.id, {
			kind: "task",
			taskId: target.taskId
		});
		else attentionEntities.set(item.id, {
			kind: "schedule",
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
		...
		config.showGoals && domain.goal?.objective !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
			className: "gj-section",
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							...
							children: domain.goal.objective
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: "gj-badge",
							children: domain.goal.phase ?? "active"
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: "gj-meta",
							children: `round ${domain.goal.roundsStarted ?? 0}`
						})
					]
				})
			})]
		}) : null,
		config.showAttention ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
			className: "gj-section",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", {
				className: "gj-sectionTitle",
				...
				className: "gj-quiet",
				children: "Nothing requires human attention."
			}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("ul", {
				className: "gj-list",
				children: [attention.map((item) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
					className: "gj-listRow gj-attention",
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						className: "gj-linkButton gj-rowMain",
						onClick: () => {
							onOpen(attentionEntities.get(item.id));
						},
						children: item.explanation
					})]
				}, item.id)), config.showTeams && !domain.teamAvailable ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", {
					className: "gj-listRow gj-warning",
					children: "Agent Teams adapter unavailable; Team tabs and controls are hidden."
				}) : null]
			})]
		}) : null,
		config.showRecentChanges ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(WhatChanged, {
			domain,
			storage: props.storage,
			onOpen
		}) : null,
		config.showUsage && domain.usage !== void 0 && usageRows(domain.usage).length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
			className: "gj-section",
			children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", {
					className: "gj-sectionTitle",
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dl", {
					...
					children: usageRows(domain.usage).map((row) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "gj-listRow gj-usageRow",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: row.label }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", {
							title: String(row.tokens),
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					className: "gj-quiet",
					children: "Provider-reported accounting for this Session's durable log; descendant totals are not exposed by current seams."
				})
			]
		}) : null,
...
function AgentEditor(props) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
		...
		/* @__PURE__ */ (0, react_jsx_runtime.jsx)(WhyIdle, {
			agent,
			domain,
			onOpen: props.onOpen
		}),
...
function WhyIdle(props) {
	...
	if (props.agent.activity !== "running") {
		...
		for (const wait of pending) reasons.push(/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
			className: "gj-listRow",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
				type: "button",
				className: "gj-linkButton gj-rowMain",
				...
				children: `Waiting on ${wait.id}`
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "gj-meta",
				children: `mode ${wait.mode} · ${wait.leaves.filter((leaf) => leaf.result === void 0).length}/${wait.leaves.length} leaves unresolved`
			})]
		}, `wait:${wait.id}`));
	}
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", {
			className: "gj-sectionTitle",
			children: "Why idle?"
		}), reasons.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
			className: "gj-quiet",
			children: "No authoritative blocker derives from current projections."
		}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
			className: "gj-list",
			"aria-label": "Deterministic idle reasons",
			children: reasons
		})]
	});
}
...
function GoalEditor(props) {
	...
	if (goal.objective === void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(UnavailableEditor, { entity: { kind: "goal" } });
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
		/* @__PURE__ */ (0, react_jsx_runtime.jsx)(EditorTitle, {
			title: goal.objective,
			subtitle: `Session goal${goal.id === void 0 ? "" : ` ${goal.id}`} · revision ${goal.revision ?? "?"}`,
			status: goal.phase ?? "active",
			entity: { kind: "goal" },
			onOpenSide: props.onOpenSide
		}),
		goal.blockedReason === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
			className: "gj-section",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", {
				className: "gj-sectionTitle",
				children: "Blocked"
				...
				className: "gj-warning",
				children: `${goal.blockedReason.code}: ${goal.blockedReason.message}`
			})]
		}),
		/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("dl", {
			className: "gj-fields",
			children: [
				goal.maxGoalRounds === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Round cap" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: String(goal.maxGoalRounds) })] }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Rounds admitted" }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: String(goal.roundsStarted ?? 0) }),
				goal.createdAt === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Created" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("time", {
					dateTime: new Date(goal.createdAt).toISOString(),
					children: new Date(goal.createdAt).toLocaleString()
				}) })] }),
				goal.updatedAt === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Updated" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("time", {
					dateTime: new Date(goal.updatedAt).toISOString(),
					children: new Date(goal.updatedAt).toLocaleString()
				}) })] })
			]
		})
	] });
...
function WorkflowEditor(props) {
	const run = props.run;
	const settled = run.members.filter((member) => member.outcome !== void 0);
	const failed = run.members.filter((member) => member.outcome === "failed").length;
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
		/* @__PURE__ */ (0, react_jsx_runtime.jsx)(EditorTitle, {
			title: run.name,
			subtitle: `Workflow run ${run.id}`,
			status: run.state,
			entity: {
				kind: "workflow",
				workflowId: run.id
		...
		/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("dl", {
			className: "gj-fields",
			children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Members" }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: `${settled.length}/${run.members.length} settled` }),
				failed > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Failed members" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: String(failed) })] }) : null,
				run.stopReason === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Stop reason" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: run.stopReason })] })
			]
		}),
		/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
			className: "gj-section",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", {
				className: "gj-sectionTitle",
				children: "Members"
			}), run.members.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: "gj-quiet",
				children: "No member published yet."
			}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
				className: "gj-list",
				"aria-label": "Workflow members",
				children: run.members.map((member) => {
					...
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "gj-stateDot",
								"data-state": member.outcome ?? "running"
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [
								member.seq,
								". ",
								member.label,
								member.phase === void 0 ? "" : ` (${member.phase})`
							] }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "gj-linkButton gj-rowMain",
								...
								children: child?.label ?? member.childId
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "gj-meta",
								children: member.outcome ?? "running"
							})
						]
					}, `${run.id}:${member.seq}`);
				})
		...
		/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
			className: "gj-quiet",
			children: "Members and outcomes come from durable `tool-workflow` Session events; timing fields are not recorded by the source events."
		})
	] });
}
function ScheduleEditor(props) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
		...
		/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("dl", {
			className: "gj-fields",
			children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Rule" }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: item.kind === "after" ? `${item.delayedSeconds ?? "?"} seconds after creation` : item.kind === "every" ? `every ${item.everySeconds ?? "?"} seconds` : "at an absolute time" }),
				item.scheduledAt === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Next target" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("time", {
					dateTime: new Date(item.scheduledAt).toISOString(),
					children: new Date(item.scheduledAt).toLocaleString()
				}) })] }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Delivery" }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: "session-local (the original session must be live)" })
			]
		}),
		/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
			className: "gj-quiet",
			children: "Records fold from durable `schedule/change` Session events; GoodJob projects them read-only and owns no scheduling authority."
		})
	] });
}
...
function apply(ctx, config = {}) {
	const resolved = {
		...
		showGoals: config.showGoals ?? DEFAULTS.showGoals,
