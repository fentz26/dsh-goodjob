...
function parseGroup(value) {
	const group = record$2(value, "group");
	return {
...
function parseChange(value) {
	const change = record$2(value, "change");
...
function parseRecord(kind, item) {
	if (typeof item.id !== "string" || typeof item.prompt !== "string") return void 0;
	if (kind === "after") {
		if (typeof item.delayedSeconds !== "number" || !Number.isSafeInteger(item.delayedSeconds)) return void 0;
		return {
			id: item.id,
			kind: "after",
			prompt: item.prompt,
	...
	if (kind === "at") {
		if (!isRfc3339Utc(item.scheduledAt)) return void 0;
		return {
			id: item.id,
			kind: "at",
			prompt: item.prompt,
	...
	if (kind === "every") {
		if (typeof item.everySeconds !== "number" || !Number.isSafeInteger(item.everySeconds)) return void 0;
		if (!isRfc3339Utc(item.scheduledAt)) return void 0;
		return {
			id: item.id,
			kind: "every",
...
function isScheduleChange(value) {
	return record$1(value)?.type === "schedule/change" && value.version === 1;
}
/** Structural face used internally when matching one stored record. */
function sameRecord(a, b) {
	return a.kind === b.kind && a.prompt === b.prompt && a.scheduledAt === b.scheduledAt && a.delayedSeconds === b.delayedSeconds && a.everySeconds === b.everySeconds;
}
/**
...
*/
function applyScheduleEvent(state, event) {
	if (!isScheduleChange(event)) return state ?? null;
	const previous = state ?? NO_SCHEDULES;
	let result = previous;
	switch (event.operation) {
		case "create": {
			const payload = record$1(event.schedule);
			if (payload === void 0) return previous;
			const parsed = parseRecord(String(payload.kind), payload);
			if (parsed === void 0) return previous;
		...
		case "delete": {
			...
			result = { schedules: previous.schedules.filter((item) => item.id !== id) };
			break;
		}
		case "dispatch": {
			const id = event.id;
			...
			result = { schedules: previous.schedules.map((item) => {
				if (item.id !== id || item.dispatched) return item;
				if (item.kind === "every" && isRfc3339Utc(acceptedAt)) return {
					...item,
					scheduledAt: nextEveryTarget(item.scheduledAt, item.everySeconds, acceptedAt)
				};
				return {
					...item,
					dispatched: true
...
function sameProjection(a, b) {
	return a === b || a.schedules.length === b.schedules.length && a.schedules.every((item, index) => {
		...
		return item.id === other.id && item.kind === other.kind && item.prompt === other.prompt && item.dispatched === other.dispatched && item.scheduledAt === other.scheduledAt && item.delayedSeconds === other.delayedSeconds && item.everySeconds === other.everySeconds;
	});
}
...
function nextEveryTarget(scheduledAt, everySeconds, acceptedAt) {
	const anchor = parseInstant(scheduledAt);
	const now = parseInstant(acceptedAt);
	const step = Math.max(1, everySeconds) * 1e3;
	const elapsed = now - anchor;
	return formatInstant(anchor + Math.max(1, Math.ceil(elapsed / step)) * step);
}
//#endregion
//#region src/rpc.ts
...
function isRunStart(value) {
	return record(value)?.type === "tool-workflow/run-start" && typeof value.runId === "string" && typeof value.name === "string";
}
/** Narrow one raw session-log record to a workflow member publish. */
function isAgentStart(value) {
	const item = record(value);
	if (item?.type !== "tool-workflow/agent-start") return false;
	return typeof item.runId === "string" && typeof item.seq === "number" && Number.isSafeInteger(item.seq) && typeof item.label === "string" && (item.phase === void 0 || typeof item.phase === "string") && typeof item.childId === "string";
}
/** Narrow one raw session-log record to a member settlement. */
function isAgentEnd(value) {
	const item = record(value);
	if (item?.type !== "tool-workflow/agent-end") return false;
	return typeof item.runId === "string" && typeof item.seq === "number" && Number.isSafeInteger(item.seq) && (item.outcome === "completed" || item.outcome === "failed" || item.outcome === "cancelled");
}
/** Narrow one raw session-log record to a terminal run event. */
function isRunEnd(value) {
	const item = record(value);
	if (item?.type !== "tool-workflow/run-end") return false;
	return typeof item.runId === "string" && (item.stopReason === "completed" || item.stopReason === "cancelled" || item.stopReason === "error");
}
/**
...
function applyWorkflowEvent(state, event) {
	const previous = state ?? NO_WORKFLOWS;
	if (isRunStart(event)) {
	...
	if (isAgentStart(event)) return takeMapped(state, mapRuns(previous, event.runId, (run) => {
		...
		return {
			...run,
			members: [...run.members, member]
		};
	}));
	if (isAgentEnd(event)) return takeMapped(state, mapRuns(previous, event.runId, (run) => {
		let changed = false;
		const members = run.members.map((member) => member.seq === event.seq && member.outcome !== event.outcome ? (changed = true, {
			...member,
	...
	if (isRunEnd(event)) return takeMapped(state, mapRuns(previous, event.runId, (run) => run.state === "running" ? {
		...run,
...
function takeMapped(state, mapped) {
...
function mapRuns(previous, runId, mutate) {
	let changed = false;
...
const ConfigSchema = z.object({
	...
	showGoals: z.boolean().default(DEFAULTS.showGoals),
	showWorkflows: z.boolean().default(DEFAULTS.showWorkflows),
...
const workflowMemberSchema = z$1.object({
	seq: z$1.number().int(),
	label: z$1.string(),
	phase: z$1.string().optional(),
	childId: z$1.string(),
	outcome: z$1.enum([
		"completed",
		"failed",
...
const goodJobWorkflowsSchema = z$1.union([z$1.object({ runs: z$1.array(z$1.object({
	id: z$1.string(),
	name: z$1.string(),
	state: z$1.enum([
		"running",
	...
	members: z$1.array(workflowMemberSchema)
})) }), z$1.null()]);
const scheduleRecordSchema = z$1.object({
	id: z$1.string(),
	kind: z$1.enum([
		"after",
		"at",
	...
	prompt: z$1.string(),
	scheduledAt: z$1.string().optional(),
	delayedSeconds: z$1.number().int().optional(),
	everySeconds: z$1.number().int().optional(),
	dispatched: z$1.boolean()
});
const goodJobSchedulesSchema = z$1.union([z$1.object({ schedules: z$1.array(scheduleRecordSchema) }), z$1.null()]);
/**
