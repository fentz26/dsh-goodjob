...
describe('GoodJob host lifecycle', () => {
  ...
  it('degrades with diagnostics when both seams are absent', async () => {
    ...
    const ctx = new Context()
    await ctx.plugin(GoodJobService)
    expect(err).toHaveBeenCalledWith(expect.stringContaining('sessionProjections not composed at load'))
    expect(err).toHaveBeenCalledWith(expect.stringContaining('settings not composed at load'))
    await ctx.fiber.dispose()
